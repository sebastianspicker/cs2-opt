...
rem
rem package-layout.txt is the release payload boundary. assets\video.txt is an
rem operator guidance artifact only and is never a CS2 video-file template. The
...
:copy_layout_entry
if /i "%~1"=="frametime.exe" (
    call :copy_one "target\x86_64-pc-windows-msvc\release\frametime.exe" "%~1"
    exit /b !errorlevel!
)
if /i "%~1"=="frametime-gui.exe" (
    call :copy_one "target\x86_64-pc-windows-msvc\release\frametime-gui.exe" "%~1"
    exit /b !errorlevel!
)
call :copy_one "%~1" "%~1"
exit /b !errorlevel!
...
:validate_layout
set "layout_count=0"
for /f "usebackq delims=" %%F in ("%source%\package-layout.txt") do (
    set /a layout_count+=1
    call :validate_layout_entry "%%F" || exit /b 1
)
if "!layout_count!"=="0" (
    echo Package layout must list at least one payload file. 1>&2
    exit /b 1
)
exit /b 0
...
if "%~1"=="" (
    echo Package layout contains an empty path. 1>&2
    exit /b 1
)
if /i "%~1"=="frametime.exe" exit /b 0
if /i "%~1"=="frametime-gui.exe" exit /b 0
if not exist "%source%\%~1" (
    echo Package layout references a missing source file: %~1 1>&2
    exit /b 1
)
exit /b 0
...
if not "!payload_count!"=="!layout_count!" (
    echo Package payload must match package-layout.txt; found !payload_count! files for !layout_count! layout entries. 1>&2
    exit /b 1
)
exit /b 0
...
set /a expected_tree_count=layout_count+1
if /i "%package_mode%"=="release" set /a expected_tree_count+=1
...
if not "!manifest_error!"=="0" (
    echo In-package manifest does not exactly match the package-layout payload files, sizes, and SHA-256 hashes. 1>&2
    exit /b 1
)
exit /b 0
