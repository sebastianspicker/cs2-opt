...
After Step 28 (1ms timer resolution), the scheduler operates at finer granularity, but a 1ms audio buffer is still extremely tight. The suite uses `0.05` (50ms) — the community competitive standard. This is well below the point where audio delay is perceptible in gameplay, and robust against CPU scheduling jitter.
