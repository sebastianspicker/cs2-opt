...
fn archive_source_and_export_zip() {
    ...
    // Use the real package-root pipeline so strip semantics see the full catalog.
...
#[test]
fn materialize_embedded_manifest() {
    let dest = unique_work().join("emb");
    fs::create_dir_all(dest.parent().unwrap()).unwrap();
    let copied = crate::copy::materialize_embedded(&dest).expect("materialize");
    let manifest = dest.join("README.md");
    assert!(copied.contains(&manifest), "should copy embedded manifest");
    assert_eq!(
        fs::read(&manifest).expect("read copied embedded manifest"),
        fs::read(driver_foundry_common::resolve_data_root().join("embedded/README.md"))
            .expect("read source embedded manifest")
    );
...
fn deep_inf_and_extended_tweaks_write_markers() {
    ...
    // The synthetic package contains Display.Driver/sample.inf for deep-INF rewriting.
