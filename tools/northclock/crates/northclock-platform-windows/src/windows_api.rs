...
use super::topology_validation::{validate_topology_record_extent, TopologyRecordExtent};
...
pub(crate) fn physical_core_count() -> Result<usize> {
    ...
    while offset < returned {
        validate_topology_record_extent(TopologyRecordExtent {
            allocation_address: base.addr(),
            allocation_size,
            returned_size: returned,
            record_offset: offset,
            record_size: header_size,
            minimum_record_size: header_size,
            record_alignment,
        })?;
