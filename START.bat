...
:restore
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Set-StrictMode -Version Latest; $ScriptRoot='%~dp0'; . '%~dp0config.env.ps1'; . '%~dp0helpers.ps1'; $SCRIPT:Mode='CONTROL'; $SCRIPT:LogLevel='NORMAL'; Restore-Interactive"
pause
goto :menu

:backupsummary
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Set-StrictMode -Version Latest; $ScriptRoot='%~dp0'; . '%~dp0config.env.ps1'; . '%~dp0helpers.ps1'; $SCRIPT:Mode='CONTROL'; $SCRIPT:LogLevel='NORMAL'; Show-BackupSummary"
pause
goto :menu
