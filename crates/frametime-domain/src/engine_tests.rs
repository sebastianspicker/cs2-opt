...
// Engine recovery invariants are exercised with a synthetic mutating P1:3.
// The live August 2026 catalog intentionally exposes P1:3 as advisory because
// routine shader-cache deletion is repair work, not an optimization default.
const REBUILDABLE_TEST_STEP: Step = Step {
    id: StepId::new(Phase::One, 3),
    category: "GPU",
    title: "Synthetic shader-cache repair",
    tier: 1,
    risk: Risk::Safe,
    depth: Depth::Filesystem,
    check_only: false,
    reboot: false,
    intent: ActionIntent::ShaderCacheHealth,
    operation: OperationKind::Filesystem,
    gpu_applicability: GpuApplicability::Any,
    orchestration_role: OrchestrationRole::Engine,
};
...
fn run(mock: Mock) -> (Result<RunReport, EngineError>, Mock, Progress) {
    ...
    let result = engine.run(&[REBUILDABLE_TEST_STEP], Profile::Yolo);
...
fn run_evidence(mock: Mock) -> (Result<RunReport, EngineError>, Mock, Progress) {
    let step = STEPS
        .iter()
        .find(|step| step.id.phase as u8 == 1 && step.id.number == 21)
        .copied()
        .expect("P1:21");
    let mut engine = Engine::new(mock, Progress::default());
...
fn p3_6_is_check_only_and_never_enters_the_mutation_path() {
    let step = *STEPS
        .iter()
        .find(|step| step.id.phase as u8 == 3 && step.id.number == 6)
...
fn cancellation_at_the_step_boundary_prevents_rebuildable_audit_and_mutation() {
    ...
    let error = engine
        .run_with_control(&[REBUILDABLE_TEST_STEP], Profile::Yolo, |_| true, || true)
...
fn skip_progress_persistence_failure_does_not_record_the_step_before_retry() {
    ...
    let error = engine
        .run(&[REBUILDABLE_TEST_STEP], Profile::Yolo)
