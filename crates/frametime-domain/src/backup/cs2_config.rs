...
pub const CS2_OPTIONAL_CONFIG_TRANSACTION_STEP: &str = "CS2:CFG";
...
impl BackupEntry {
    ...
    /// Captures an exact, closed set for a one-shot optional CFG deployment.
    /// This transaction is independent from P1:34 progress and recovery.
    pub fn capture_optional_cs2_config_transaction(
        install: &Cs2Install,
        ...
        let controller = Cs2ConfigController::new(install.clone())?;
        let target_paths = controller.backup_optional_targets(assets)?;
        let entry = capture_targets(
            controller.install(),
        ...
        entry.validate_optional_cs2_config_transaction(install, assets)?;
        Ok(entry)
    }
    ...
    pub fn restore_optional_cs2_config_transaction(
        &self,
        install: &Cs2Install,
        files: &mut dyn Cs2ConfigFs,
    ) -> Result<(), Cs2ConfigBackupError> {
    ...
    /// Applies a captured optional transaction only while every selected target
    /// still exactly matches the persisted pre-mutation snapshot.
    pub fn apply_optional_cs2_config_transaction(
        &self,
        install: &Cs2Install,
        files: &mut dyn Cs2ConfigFs,
...
fn capture_targets(
    ...
    step: &'static str,
    captured_at: &str,
    target_paths: Vec<(Cs2ConfigTarget, std::path::PathBuf)>,
    files: &mut dyn Cs2ConfigFs,
) -> Result<BackupEntry, Cs2ConfigBackupError> {
    ...
    Ok(BackupEntry::Cs2ConfigTransaction {
        step: step.to_owned(),
        timestamp: captured_at.to_owned(),
        install_identity: install_identity(install),
        targets,
        unknown: BTreeMap::new(),
    })
}

fn optional_assets_from_snapshots(
    targets: &[Cs2ConfigSnapshot],
) -> Result<BTreeSet<crate::cs2_config::OptionalCfgAsset>, Cs2ConfigBackupError> {
    targets
        .iter()
        .map(|snapshot| {
            snapshot
                .target
                .optional_asset()
                .ok_or(Cs2ConfigBackupError::UnexpectedTarget(snapshot.target))
        })
        .collect()
}
