...
fn run_irreversible(
    ...
    let step = if phase == 1 && number == 13 {
        Step {
            id: StepId::new(Phase::One, 13),
            category: "System",
            title: "Synthetic app-removal repair",
            tier: 2,
            risk: Risk::Moderate,
            depth: Depth::App,
            check_only: false,
            reboot: false,
            intent: ActionIntent::BackgroundAppGuidance,
            operation: OperationKind::ApplicationConfiguration,
        ...
        *STEPS
            .iter()
            .find(|step| step.id.phase as u8 == phase && step.id.number == number)
            .expect("catalog step")
    };
