...
// Repair actions are intentionally absent from Quick and Full. Callers must
// expose them as a separately acknowledged diagnostic or repair workflow;
// routine cleanup must not discard caches or diagnostic evidence, reset
// network state, or trigger a game-file validation.
const REPAIR_ACTIONS: &[CleanupActionSpec] = &[
    CleanupActionSpec {
...
/// baseline, never the explicitly selected repair actions. Driver performs no
/// cleanup actions until its runtime handoff is available; it does not silently
/// fall through to the Full action set.
#[must_use]
...
/// Explicit repair actions that must not be presented as an optimization
/// preset. The caller is responsible for a repair-specific acknowledgement.
#[must_use]
pub fn repair_actions() -> Vec<CleanupActionSpec> {
    REPAIR_ACTIONS.to_vec()
}
...
mod tests {
    ...
    #[test]
    fn automatic_cleanup_excludes_repair_actions_without_relaxing_full_acknowledgement() {
        let actions = cleanup_actions(CleanupMode::Full);
        assert!(actions.iter().all(|spec| !REPAIR_ACTIONS.contains(spec)));
        assert!(requires_irreversible_acknowledgement(CleanupMode::Full));

        let repairs = repair_actions();
        assert!(repairs.iter().any(|spec| {
