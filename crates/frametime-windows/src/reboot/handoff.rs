...
pub fn arm_safe_mode_handoff(runtime: &VerifiedSelectedRuntime) -> Result<(), String> {
...
pub fn complete_phase_two_safe_boot_clear(runtime: &VerifiedSelectedRuntime) -> Result<(), String> {
    let trusted = TrustedWorkDir::acquire_fixed()?;
    let work_dir = trusted.path();
    let _lock = WorkLock::acquire(work_dir)?;
...
pub fn arm_phase_three_handoff(runtime: &VerifiedSelectedRuntime) -> Result<(), String> {
    ...
    let work_dir = trusted.path();
    let _lock = WorkLock::acquire(work_dir)?;
    let driver = load_driver_transaction_at(work_dir)?
        .ok_or("P2:3 requires durable P1:18/P1:19 NVIDIA transaction evidence")?;
    if !driver.removal_complete() {
        return Err(
            "P2:3 requires coherent P2:2 removal evidence; retain the Phase 2 handoff".into(),
        );
    }
    let (mut state, progress) = read_handoff_session(work_dir)?;
...
pub fn relaunch_phase_three_handoff(runtime: &VerifiedSelectedRuntime) -> Result<(), String> {
    let trusted = TrustedWorkDir::acquire_fixed()?;
    let work_dir = trusted.path();
    let _lock = WorkLock::acquire(work_dir)?;
    let (state, progress) = read_handoff_session(work_dir)?;
...
pub fn clear_phase_three_handoff(runtime: &VerifiedSelectedRuntime) -> Result<(), String> {
    let trusted = TrustedWorkDir::acquire_fixed()?;
    let work_dir = trusted.path();
    let _lock = WorkLock::acquire(work_dir)?;
    let (state, progress) = read_handoff_session(work_dir)?;
...
pub(crate) fn new_transaction(
    ...
    sid: String,
) -> Result<RebootTransaction, String> {
...
pub(crate) fn require_runtime_and_stage(
...
pub(crate) fn read_handoff_session(work_dir: &Path) -> Result<(State, Progress), String> {
...
pub(crate) fn read_state_for_handoff(
...
pub(crate) fn read_progress_for_handoff(
    trusted: &TrustedWorkDir,
    work_dir: &Path,
...
pub(crate) fn persist_state_readback(
    work_dir: &Path,
    state: &State,
    prefix: &str,
) -> Result<(), String> {
...
pub(crate) fn persist_progress_readback(
    work_dir: &Path,
    progress: &Progress,
    prefix: &str,
) -> Result<(), String> {
...
pub(crate) fn handoff_command(executable: &Path, arguments: &str) -> Result<String, String> {
    let path = executable
        .to_str()
...
pub(crate) fn read_handoff_value(
    hive: Hive,
    key: &'static str,
    name: &'static str,
...
pub(crate) fn ensure_handoff_value(
    hive: Hive,
    key: &'static str,
    name: &'static str,
    expected: &str,
...
pub(crate) fn require_exact_handoff(
    hive: Hive,
    key: &'static str,
    name: &'static str,
    expected: &str,
...
pub(crate) fn set_safeboot_minimal() -> Result<(), String> {
    CommandVector::new(
        CommandName::Bcdedit,
...
pub(crate) fn clear_safeboot() -> Result<(), String> {
    CommandVector::new(
        CommandName::Bcdedit,
...
pub(crate) fn final_benchmark_status_locked(
    work_dir: &Path,
    state: &State,
    progress: &Progress,
) -> Result<(), String> {
...
#[cfg(windows)]
pub(crate) fn random_transaction_id() -> Result<TransactionId, String> {
    use std::ffi::c_void;
...
#[cfg(windows)]
pub(crate) fn shell_execute_phase_three(executable: &Path) -> Result<(), String> {
...
#[cfg(not(windows))]
pub(crate) fn shell_execute_phase_three(_: &Path) -> Result<(), String> {
...
mod reboot_handoff_tests {
    ...
    #[test]
    pub(crate) fn exact_handoff_command_quotes_the_selected_executable_without_a_shell() {
