#[cfg(windows)]
use super::capability::{authorization_expiry_after, validate_bounded_nvidia_authorization};
#[cfg(windows)]
use super::transaction::persist_driver_transaction;
use crate::*;
#[cfg(windows)]
use frametime_domain::driver::{
    ArtifactAcquisitionAuthorization, ArtifactIdentity, ArtifactLocator, CaptureFreshnessPolicy,
    DriverPlanInput, ExecutionClock, GpuVendor as DriverGpuVendor, InspectionAdapter,
    InstallationEvidence, SCHEMA_VERSION, generate_dry_run_plan,
};
...
pub fn remove_prepared_nvidia_driver(
    ...
    let evidence = frametime_domain::driver::remove_captured_packages(
        &transaction.plan,
        capture.clone(),
        &removal,
        &NativeDriverClock,
...
pub fn install_prepared_nvidia_driver(
    ...
    let capability = store
        .acquire(
            &NvidiaArtifactLocation::LocalLeaf(
                transaction.artifact.locator.artifact_file_name.clone(),
            ),
            &format!(
                "driver-artifacts/{}",
                transaction.artifact.locator.artifact_file_name
            ),
            2 * 1024 * 1024 * 1024,
        )
        .map_err(|error| error.to_string())?;
