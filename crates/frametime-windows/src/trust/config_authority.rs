...
#[derive(Debug)]
pub(crate) struct VerifiedConfigInner {
    ...
    pub(crate) value: Config,
    pub(crate) identity: ConfigIdentity,
}
...
impl VerifiedConfig {
    ...
    pub(crate) fn from_verified_bytes(
        ...
        let actual_size =
            u64::try_from(bytes.len()).map_err(|_| "verified configuration size overflows u64")?;
...
mod config_authority_tests {
    ...
    #[test]
    pub(crate) fn verified_config_is_cloneable_and_snapshot_bound() {
        ...
        assert_eq!(clone.value().fps_cap, config.value().fps_cap);
    }

    #[test]
    pub(crate) fn verified_config_rejects_identity_or_content_tampering() {
        let bytes = fixture();
        let digest = format!("{:x}", Sha256::digest(&bytes));
        assert!(
            VerifiedConfig::from_verified_bytes(bytes.clone(), bytes.len() as u64 + 1, &digest)
                .is_err()
        );
        assert!(
            VerifiedConfig::from_verified_bytes(bytes.clone(), bytes.len() as u64, "00").is_err()
        );
        assert!(
            VerifiedConfig::from_verified_bytes(
                vec![0xff],
                1,
                &format!("{:x}", Sha256::digest([0xff]))
            )
            .is_err()
        );
    }
