...
mod trusted_acl_validation {
    ...
    pub(crate) fn read_u16(bytes: &[u8], offset: usize, field: &str) -> Result<u16, String> {
        let end = offset
    ...
    pub(crate) fn read_u32(bytes: &[u8], offset: usize, field: &str) -> Result<u32, String> {
        let end = offset
...
mod trusted_acl_validation_tests {
    ...
    #[test]
    pub(crate) fn acl_extent_accepts_exactly_contained_aces() {
        let acl = valid_acl();
    ...
    #[test]
    pub(crate) fn acl_extent_rejects_declared_size_outside_buffer() {
    ...
    #[test]
    pub(crate) fn acl_extent_rejects_ace_and_sid_overruns() {
    ...
    #[test]
    pub(crate) fn acl_extent_rejects_misaligned_sid_address() {
        let acl = valid_acl();
        assert_eq!(
...
pub(crate) mod trusted_work_dir {
    ...
    pub(crate) fn open_or_create(requested: &Path) -> Result<(PathBuf, RootHandle), String> {
    ...
    pub(crate) fn validate_child_handle(handle: HANDLE, leaf: &str) -> Result<(), String> {
        if !crate::trust::json_common::is_allowed_child(leaf) {
            return Err("suite child identity is not allowlisted".into());
        }
        validate_descendant_handle(handle, leaf, false)
    }

    pub(crate) fn validate_temporary_handle(handle: HANDLE, leaf: &str) -> Result<(), String> {
        if !crate::trust::json_common::is_temporary_leaf(leaf) {
    ...
    pub(crate) fn validate_descendant_handle(
        handle: HANDLE,
        relative: &str,
        directory: bool,
    ) -> Result<(), String> {
    ...
    pub(crate) fn safe_descendant_path(relative: &str) -> bool {
        !relative.is_empty()
    ...
    pub(crate) fn harden_created_child(handle: HANDLE) -> Result<(), String> {
        apply_exact_security(handle)?;
        validate_exact_security(handle)
    }

    pub(crate) fn open_driver_artifact_directory(root: HANDLE) -> Result<RootHandle, String> {
    ...
    pub(crate) fn open_directory(path: &str) -> Result<RootHandle, String> {
        let path = wide(path);
        let handle = unsafe {
            CreateFileW(
                PCWSTR(path.as_ptr()),
    ...
    pub(crate) fn create_protected_root(path: &str) -> Result<bool, String> {
    ...
    pub(crate) struct LocalSecurityDescriptor(PSECURITY_DESCRIPTOR);
    ...
    pub(crate) fn security_descriptor_from_sddl() -> Result<LocalSecurityDescriptor, String> {
    ...
    pub(crate) struct ProtectedSecurityAttributes {
    ...
    pub(crate) fn validate_open_root(handle: RootHandle) -> Result<(PathBuf, RootHandle), String> {
    ...
    pub(crate) fn reject_intermediate_reparse_points() -> Result<(), String> {
    ...
    pub(crate) fn reject_reparse_point(handle: HANDLE) -> Result<(), String> {
        let mut info = FILE_ATTRIBUTE_TAG_INFO::default();
        unsafe {
            GetFileInformationByHandleEx(
                handle,
    ...
    pub(crate) fn final_dos_path(handle: HANDLE) -> Result<String, String> {
    ...
    pub(crate) fn validate_exact_security(handle: HANDLE) -> Result<(), String> {
    ...
    pub(crate) fn apply_exact_security(handle: HANDLE) -> Result<(), String> {
        use windows::Win32::{
    ...
    pub(crate) struct WellKnownSid(Vec<u8>);
    impl WellKnownSid {
        fn as_psid(&self) -> PSID {
            PSID(self.0.as_ptr().cast_mut().cast::<c_void>())
        }
    }
    pub(crate) fn well_known_sid(
