...
use crate::{
    ...
    io_contract::{is_missing_path_hresult, parse_export_destination},
    trusted_work_dir,
};

pub(crate) const LOG_DIRECTORY: &str = "Logs";
pub(crate) const CURRENT_LOG: &str = "Logs\\frametime_current.log";
pub(crate) const COPY_BUFFER_BYTES: usize = 64 * 1024;
pub(crate) const MAX_LOG_BYTES: usize = 16 * 1024 * 1024;
pub(crate) const MAX_BACKUP_BYTES: usize = 64 * 1024 * 1024;

#[derive(Debug)]
pub(crate) struct RetainedHandle(HANDLE);
...
pub(crate) fn open_trusted_logs_directory(
    trusted: &TrustedWorkDir,
...
pub(crate) fn open_trusted_file(
    trusted: &TrustedWorkDir,
    relative: &str,
) -> Result<RetainedHandle, String> {
    trusted_work_dir::validate_exact_security(trusted.root_handle())?;
...
pub(crate) fn open_destination_parents(
    destination: &super::io_contract::ExportDestination,
...
pub(crate) fn open_directory(path: &str) -> Result<RetainedHandle, String> {
    let path = wide(path);
    let handle = unsafe {
        CreateFileW(
            PCWSTR(path.as_ptr()),
...
pub(crate) fn require_fixed_local_drive(root: &str) -> Result<(), String> {
    let root = wide(root);
...
pub(crate) fn validate_directory(handle: HANDLE, expected: &str) -> Result<(), String> {
...
pub(crate) fn validate_regular_file(handle: HANDLE, label: &str) -> Result<(), String> {
    validate_type(handle, false, label)
}

pub(crate) fn validate_type(handle: HANDLE, directory: bool, label: &str) -> Result<(), String> {
...
pub(crate) fn validate_final_path(
    handle: HANDLE,
    expected: &str,
    label: &str,
) -> Result<(), String> {
...
pub(crate) fn final_path(handle: HANDLE) -> Result<String, String> {
...
pub(crate) fn same_windows_path(actual: &str, expected: &str) -> bool {
    actual
        .strip_prefix(r"\\?\")
        .unwrap_or(actual)
...
pub(crate) fn bounded_length(handle: HANDLE, maximum: usize, label: &str) -> Result<usize, String> {
...
pub(crate) fn read_bounded(handle: HANDLE, maximum: usize, label: &str) -> Result<Vec<u8>, String> {
...
pub(crate) fn hash_handle(handle: HANDLE, length: usize, label: &str) -> Result<[u8; 32], String> {
    seek_start(handle, label)?;
...
pub(crate) fn read_exact(handle: HANDLE, bytes: &mut [u8], label: &str) -> Result<(), String> {
    let mut offset = 0;
...
pub(crate) fn write_all(handle: HANDLE, bytes: &[u8]) -> Result<(), String> {
    let mut offset = 0;
    while offset < bytes.len() {
...
pub(crate) fn seek_start(handle: HANDLE, label: &str) -> Result<(), String> {
