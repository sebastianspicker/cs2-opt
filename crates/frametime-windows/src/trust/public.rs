use crate::*;
pub(crate) fn backup_entry_kind(entry: &BackupEntry) -> &'static str {
    match entry {
...
pub struct TrustedWorkDir {
    ...
    #[cfg(windows)]
    pub(crate) _handle: trusted_work_dir::RootHandle,
}
...
pub(crate) fn requested_root_is_exact(work_dir: &Path) -> bool {
...
pub(crate) fn write_json_atomic_trusted<T: serde::Serialize>(
    ...
    #[cfg(windows)]
    {
        super::json_windows::write_json_atomic(trusted, name, value)
    }
    #[cfg(not(windows))]
    {
...
pub(crate) fn read_json_trusted<T: serde::de::DeserializeOwned>(
    trusted: &TrustedWorkDir,
    name: &str,
) -> Result<T, String> {
    #[cfg(windows)]
    {
        super::json_windows::read_json(trusted, name)
    }
    #[cfg(not(windows))]
    {
        let _ = (trusted, name);
