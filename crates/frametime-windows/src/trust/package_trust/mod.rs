...
impl PublisherPinConfiguration {
    ...
    pub fn parse(value: &str) -> Result<Self, String> {
        contract::parse_publisher_pins(value).map(Self::Configured)
    }
...
pub struct AuthenticatedExecutable {
    ...
    #[cfg(windows)]
    pub(crate) _retained: self::windows::RetainedFile,
}
...
pub struct AuthenticatedPackage {
    ...
    pub(crate) gui: AuthenticatedExecutable,
    pub(crate) cli: AuthenticatedExecutable,
    #[cfg(windows)]
    pub(crate) payload: std::collections::BTreeMap<String, self::windows::RetainedFile>,
    #[cfg(windows)]
    pub(crate) _retained: Vec<self::windows::RetainedFile>,
}
...
pub fn authenticate_current_package() -> Result<AuthenticatedPackage, String> {
    ...
    #[cfg(windows)]
    {
        ...
        self::windows::authenticate(root, &pins, &image)
    }
    #[cfg(not(windows))]
    {
        let _ = pins;
...
pub(crate) fn ensure_release_feature_set() -> Result<(), String> {
...
mod release_feature_tests {
    ...
    #[test]
    pub(crate) fn qualification_only_builds_cannot_mint_package_authority() {
        assert_eq!(
