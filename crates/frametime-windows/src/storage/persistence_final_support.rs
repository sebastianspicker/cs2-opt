...
#[cfg(windows)]
pub(crate) fn random_final_receipt_id() -> Result<TransactionId, String> {
    use std::ffi::c_void;
...
pub(crate) fn read_state_for_final(
...
pub(crate) fn read_progress_for_final(
    trusted: &TrustedWorkDir,
    work_dir: &Path,
...
pub(crate) fn read_history_for_final(
    trusted: &TrustedWorkDir,
    work_dir: &Path,
...
pub fn final_benchmark_status() -> Result<FinalBenchmarkStatus, String> {
    final_benchmark_status_at(TrustedWorkDir::acquire_fixed()?.path())
}

pub(crate) fn final_benchmark_status_at(work_dir: &Path) -> Result<FinalBenchmarkStatus, String> {
    ...
    Ok(final_benchmark_status_from_records(
        &state, &progress, &history,
    ))
}

pub(crate) fn final_benchmark_status_from_records(
    state: &State,
    progress: &Progress,
