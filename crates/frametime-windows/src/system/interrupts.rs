...
pub(crate) const INTERRUPT_PARAMETERS_SUFFIX: &str =
    "Device Parameters\\Interrupt Management\\MessageSignaledInterruptProperties";
pub(crate) const AFFINITY_POLICY_SUFFIX: &str =
    "Device Parameters\\Interrupt Management\\Affinity Policy";
pub(crate) const MSI_SUPPORTED: &str = "MSISupported";
pub(crate) const MESSAGE_NUMBER_LIMIT: &str = "MessageNumberLimit";
pub(crate) const DEVICE_POLICY: &str = "DevicePolicy";
pub(crate) const ASSIGNMENT_SET_OVERRIDE: &str = "AssignmentSetOverride";
...
impl MsiDeviceBatch {
    pub fn build(
        ...
        if !device
            .class_guid
            .eq_ignore_ascii_case(device_class.class_guid())
        {
...
pub fn discover_nic_affinity_binding(
    ...
    let adapter =
        resolve_active_physical_wired_adapter(network_enumerator.enumerate_network_adapters()?)?;
...
pub(crate) fn interrupt_parameters_key(device: &InterruptPciDeviceBinding) -> String {
    format!(
        "SYSTEM\\CurrentControlSet\\Enum\\{}\\{INTERRUPT_PARAMETERS_SUFFIX}",
        device.instance_id
    )
}

pub(crate) fn affinity_policy_key(device: &InterruptPciDeviceBinding) -> String {
    format!(
        "SYSTEM\\CurrentControlSet\\Enum\\{}\\{AFFINITY_POLICY_SUFFIX}",
        device.instance_id
    )
}

pub(crate) fn same_pci_pnp_identity(
