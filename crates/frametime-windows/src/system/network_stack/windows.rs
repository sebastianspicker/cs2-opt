...
pub(crate) const MAX_INSTANCE_UNITS: usize = 512;
...
pub(crate) struct DriverKey(HKEY);
...
pub(crate) fn open_driver_key(
    adapter: &NetworkAdapterBinding,
    rights: u32,
) -> Result<DriverKey, String> {
    adapter
        .validate()
...
pub(crate) fn exact_device(
    set: &DeviceInfoSet,
    expected_instance: &str,
) -> Result<SP_DEVINFO_DATA, String> {
...
pub(crate) fn device_instance_id(
    set: &DeviceInfoSet,
...
pub(crate) fn verify_netcfg_identity(
    key: &DriverKey,
    adapter: &NetworkAdapterBinding,
) -> Result<(), String> {
...
pub(crate) fn utf16_bytes(bytes: &[u8], label: &str) -> Result<String, String> {
...
pub(crate) fn utf16_text(units: &[u16], label: &str) -> Result<String, String> {
...
pub(crate) fn name_label(name: &[u16]) -> String {
