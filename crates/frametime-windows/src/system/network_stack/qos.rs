...
//!
//! The automatic P1:16 transaction does not dispatch policy or NLA writes.
//! This module remains to restore exact historical snapshots and to support a
...
use crate::wmi::{
    boolean, nullable_string, on_mta, put_bool, put_sint8, put_string, put_uint8, put_uint16,
    put_uint32, put_uint64, query, require_class, services_at, sint8, string, uint8, uint16,
    uint32, uint64,
};

pub(crate) const QOS_NAMESPACE: &str = "ROOT\\StandardCimv2";
pub(crate) const QOS_CLASS: &str = "MSFT_NetQosPolicySettingData";
pub(crate) const QOS_NAME_PORTS: &str = "CS2_UDP_Ports";
pub(crate) const QOS_NAME_APP: &str = "CS2_App";
pub(crate) const QOS_NLA_KEY: &str = r"SYSTEM\CurrentControlSet\Services\Tcpip\QoS";
pub(crate) const QOS_NLA_NAME: &str = "Do not use NLA";
pub(crate) const MAX_NLA_BYTES: usize = 64;
...
pub(crate) fn write_nla_raw(
    key: &RegistryKey,
    value: &NetworkStackRawRegistryValue,
) -> Result<(), String> {
...
pub(crate) fn nla_key_is_empty(key: &RegistryKey) -> Result<bool, String> {
    let mut subkeys = 0;
    let mut values = 0;
    unsafe {
        RegQueryInfoKeyW(
...
pub(crate) fn target_nla_bytes() -> Vec<u8> {
    vec![b'1', 0, 0, 0]
}

pub(crate) fn target_nla_value() -> NetworkStackRawRegistryValue {
    NetworkStackRawRegistryValue {
        value_type: REG_SZ.0,
...
pub(crate) fn read_policy_on_mta(
    policy: NetworkStackPolicy,
) -> Result<Option<NetworkStackPolicySnapshot>, String> {
...
pub(crate) fn write_policy_on_mta(
    policy: NetworkStackPolicy,
    ...
    if read_policy_with_services(&services, policy)?.as_ref() != Some(desired) {
        return Err("P1:16 QoS provider semantic readback did not match restored policy".into());
    }
    Ok(())
}

pub(crate) fn delete_policy_on_mta(policy: NetworkStackPolicy) -> Result<(), String> {
    let services = services_at(QOS_NAMESPACE)?;
...
pub(crate) fn read_policy_with_services(
...
pub(crate) fn policy_name(policy: NetworkStackPolicy) -> &'static str {
    match policy {
