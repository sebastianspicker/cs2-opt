...
impl PciDeviceClass {
    ...
    pub(crate) fn from_class_guid(value: &str) -> Option<Self> {
...
#[cfg(windows)]
pub(crate) mod windows_setupapi {
    use super::*;
    use windows::{
        Win32::{
            Devices::{
    ...
    pub(crate) fn registry_string(key: HKEY, name: &str) -> Result<String, DeviceBindingError> {
    ...
    pub(crate) fn utf16_text(units: &[u16], label: &str) -> Result<String, DeviceBindingError> {
    ...
    pub(crate) fn parse_pci_identity(
        value: &str,
    ) -> Result<(u16, u16, u16, u16, u8), DeviceBindingError> {
    ...
    pub(crate) fn parse_hex(value: &str, width: usize) -> Option<u32> {
        (value.len() == width)
            .then(|| u32::from_str_radix(value, 16).ok())
            .flatten()
    }

    pub(crate) fn format_guid(value: GUID) -> String {
        format!("{{{value:?}}}")
    }
