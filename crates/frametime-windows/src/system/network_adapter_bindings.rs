...
mod windows_ip_helper {
    ...
    pub(crate) struct IpAdapterRecord {
        guid: String,
        luid: u64,
        interface_index: u32,
    ...
    pub(crate) fn pci_links() -> Result<BTreeMap<String, CorePciDeviceBinding>, DeviceBindingError>
    {
    ...
    pub(crate) fn adapters() -> Result<Vec<IpAdapterRecord>, DeviceBindingError> {
        ...
        {
            return Err(error(
                "GetAdaptersAddresses did not return a bounded required buffer",
            ));
        }
        for _ in 0..2 {
        ...
        Err(error(
            "GetAdaptersAddresses changed its required size repeatedly",
        ))
    }

    pub(crate) fn copy_adapter_list(
        storage: &[u64],
    ) -> Result<Vec<IpAdapterRecord>, DeviceBindingError> {
        ...
        for _ in 0..MAX_ADAPTERS {
            if current < start
                || current.checked_add(size_of::<IP_ADAPTER_ADDRESSES_LH>()) > Some(end)
            {
                return Err(error(
                    "IP Helper returned an adapter pointer outside its buffer",
                ));
            }
        ...
        Err(error(
            "IP Helper adapter list exceeds the safe record limit",
        ))
    }

    pub(crate) fn adapter_record(
        ...
        if physical_address_length > adapter.PhysicalAddress.len() {
            return Err(error(
                "network physical address length exceeds IP Helper storage",
            ));
        }
        Ok(IpAdapterRecord {
    ...
    pub(crate) fn error(message: impl Into<String>) -> DeviceBindingError {
