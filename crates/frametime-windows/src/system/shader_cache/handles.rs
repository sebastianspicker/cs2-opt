...
use crate::normalize_windows_dos_path;
...
pub(crate) fn delete_fixed_roots(paths: &[std::path::PathBuf]) -> Result<usize, String> {
    let tree = CacheTree::preflight(paths)?;
...
impl CacheTree {
    fn preflight(roots: &[std::path::PathBuf]) -> Result<Self, String> {
        let mut tree = Self {
            nodes: Vec::new(),
            roots: Vec::new(),
            memory: 0,
        ...
        for root in roots {
            let Some(handle) = open_root(root)? else {
                continue;
            };
            ...
            root_final_path_matches_configured(&tree.nodes[index].final_path, root)?;
