...
pub(crate) mod native_services {
    ...
    pub(crate) struct ScopedServiceHandle(SC_HANDLE);
    ...
    pub(crate) fn start_type(value: &str) -> Result<SERVICE_START_TYPE, String> {
        match value {
    ...
    pub(crate) fn open_manager() -> Result<ScopedServiceHandle, String> {
        unsafe { OpenSCManagerW(None, None, SC_MANAGER_CONNECT) }
            .map(ScopedServiceHandle)
            .map_err(|error| format!("open SCM: {error}"))
    }

    pub(crate) fn open_present(
        manager: SC_HANDLE,
        name: &str,
    ) -> Result<Option<ScopedServiceHandle>, String> {
        let name = wide(name);
    ...
    pub(crate) fn query_status(service: SC_HANDLE) -> Result<SERVICE_STATUS_PROCESS, String> {
    ...
    pub(crate) fn status_name(status: SERVICE_STATUS_PROCESS) -> Result<&'static str, String> {
    ...
    pub(crate) fn configure_delayed_auto_start(
        service: SC_HANDLE,
        delayed: bool,
    ) -> Result<(), String> {
    ...
    pub(crate) fn stop(service: SC_HANDLE) -> Result<(), String> {
...
#[cfg(not(windows))]
pub(crate) mod native_services {
    pub(crate) fn restore(_: &str, _: &str, _: bool, _: &str) -> Result<(), String> {
        Err("service restore requires Windows SCM".into())
    }
}
#[cfg(windows)]
pub(crate) mod native_task_scheduler {
    use windows::{
        Win32::{
...
#[cfg(not(windows))]
pub(crate) mod native_task_scheduler {
    pub(crate) fn restore(_: &str, _: &str, _: bool, _: bool) -> Result<(), String> {
