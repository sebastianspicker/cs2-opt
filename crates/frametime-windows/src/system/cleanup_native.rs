...
#[cfg(windows)]
use super::cleanup_shader::{CleanupShaderCacheKind, clear_cleanup_shader_cache};
use crate::{
    AuthenticatedPackage, Progress, VerifiedConfig, launch_published_safe_mode_handoff,
    load_driver_transaction_at, load_progress_at, publish_current_packaged_runtime,
    require_elevation,
};
#[cfg(windows)]
...
pub(crate) const STEAM_VALIDATE_730_URI: &str = "steam://validate/730";
...
pub(crate) fn driver_report(work_dir: &Path, package: &AuthenticatedPackage) -> CleanupReport {
    let outcome = (|| {
        require_elevation()?;
        let progress = load_progress_at(work_dir)?;
        require_phase_one_handoff_ready(&progress)
            .map_err(|error| format!("driver cleanup requires resolved P1:1-37: {error:?}"))?;
        let transaction = load_driver_transaction_at(work_dir)?
            .ok_or("driver cleanup requires a coherent prepared NVIDIA transaction")?;
        if transaction.plan.target_gpu.vendor != frametime_domain::driver::GpuVendor::Nvidia {
        ...
        let verified = load_progress_at(work_dir)?;
...
#[cfg(windows)]
pub(crate) fn reset_winsock_catalog() -> Result<usize, String> {
...
mod cleanup_paths {
    ...
    pub(crate) fn windows_root() -> Result<PathBuf, String> {
    ...
    pub(crate) fn user_temp_root() -> Result<PathBuf, String> {
    ...
    pub(super) fn windows_temp() -> Result<usize, String> {
        crate::system::shader_cache::handles::delete_fixed_roots(&[windows_root()?.join("Temp")])
    }

    pub(super) fn current_user_temp() -> Result<usize, String> {
        crate::system::shader_cache::handles::delete_fixed_roots(&[user_temp_root()?])
    }

    pub(super) fn prefetch_pf() -> Result<usize, String> {
        crate::system::shader_cache::handles::delete_prefetch_pf_files(
            &windows_root()?.join("Prefetch"),
        )
    }
}
...
mod tests {
    ...
    pub(crate) fn verified_config() -> VerifiedConfig {
    ...
    #[test]
    pub(crate) fn report_has_one_result_in_exact_core_order_even_when_every_host_action_is_deferred()
     {
        let report = run(
            CleanupMode::Full,
    ...
    #[test]
    pub(crate) fn fixed_steam_validation_target_is_clipboard_only() {
