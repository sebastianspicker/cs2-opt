...
pub(crate) struct MemoryChannel {
    ...
    pub(crate) device_handles: Vec<u16>,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct MemoryTopology {
    pub(crate) channel_count: u16,
    pub(crate) populated_channels: Vec<MemoryChannel>,
}

pub(crate) const RAW_SMBIOS_HEADER_SIZE: usize = 8;
#[cfg(windows)]
pub(crate) const MAX_RAW_SMBIOS_BYTES: usize = 16 * 1024 * 1024;

pub(crate) fn capture_memory_topology() -> Result<Option<MemoryTopology>, String> {
    parse_memory_topology(&read_raw_smbios()?)
}

pub(crate) fn verify_memory_topology(captured: &Option<MemoryTopology>) -> Result<(), String> {
...
pub(crate) fn raw_smbios_table(raw: &[u8]) -> Result<&[u8], String> {
    ...
    let declared =
        usize::try_from(declared).map_err(|_| "P1:24 RawSMBIOSData length overflows usize")?;
...
pub(crate) fn next_smbios_structure<'a>(
    table: &'a [u8],
    offset: &mut usize,
) -> Result<&'a [u8], String> {
    let start = *offset;
...
pub(crate) fn type17_is_populated(structure: &[u8]) -> Result<bool, String> {
    if structure.len() < 14 {
...
pub(crate) fn parse_type37_channel(handle: u16, structure: &[u8]) -> Result<MemoryChannel, String> {
    if structure.len() < 7 {
...
#[cfg(windows)]
pub(crate) fn read_raw_smbios() -> Result<Vec<u8>, String> {
    ...
    let required =
        usize::try_from(required).map_err(|_| "P1:24 SMBIOS size query overflows usize")?;
...
mod smbios_tests {
    ...
    pub(crate) fn structure(kind: u8, handle: u16, formatted: &[u8]) -> Vec<u8> {
    ...
    pub(crate) fn type17(handle: u16, size: u16) -> Vec<u8> {
        let mut formatted = vec![0; 10];
        formatted[8..10].copy_from_slice(&size.to_le_bytes());
        structure(17, handle, &formatted)
    }

    pub(crate) fn type37(handle: u16, device_handles: &[u16]) -> Vec<u8> {
    ...
    #[test]
    pub(crate) fn parses_complete_populated_channel_membership_without_inferring_mode() {
        ...
        assert_eq!(
            ...
            vec![
                MemoryChannel {
                    handle: 0x1200,
                    device_handles: vec![0x1100]
                },
                MemoryChannel {
                    handle: 0x1201,
                    device_handles: vec![0x1101]
                },
            ]
        );
        assert!(
            observation
                .populated_channels
                .iter()
                .all(|channel| !channel.device_handles.is_empty())
        );
    }

    #[test]
    pub(crate) fn absent_or_unusable_channel_mapping_is_inapplicable() {
        assert_eq!(
            parse_memory_topology(&raw(vec![type17(0x1100, 8192)])),
            Ok(None)
        );
        assert_eq!(
    ...
    #[test]
    pub(crate) fn rejects_truncation_duplicate_handles_dangling_and_duplicate_membership() {
        ...
        assert!(
            parse_memory_topology(&raw(vec![type17(0x1100, 8192), type17(0x1100, 8192)])).is_err()
        );
        assert!(parse_memory_topology(&raw(vec![type37(0x1200, &[0x1100])])).is_err());
        assert!(
            parse_memory_topology(&raw(vec![
                type17(0x1100, 8192),
                type37(0x1200, &[0x1100]),
    ...
    #[test]
    pub(crate) fn rejects_raw_header_and_type37_length_mismatches() {
