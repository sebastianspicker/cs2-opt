//! Read-only Windows implementation of [`frametime_domain::hardware`] diagnostics.
//!
...
#[cfg(not(windows))]
use frametime_domain::hardware::DiagnosticError;
#[cfg(windows)]
use frametime_domain::hardware::DiagnosticPayload;
        ...
        #[cfg(not(windows))]
        {
            DiagnosticEnvelope::failure(
                ...
                DiagnosticError::unavailable(
                    "frametime-windows hardware diagnostics only run on Windows",
                ),
            )
        }
    }
...
mod tests {
    use super::*;
    #[cfg(not(windows))]
    use frametime_domain::hardware::{DiagnosticStatus, EtwFrameCaptureRequest};
