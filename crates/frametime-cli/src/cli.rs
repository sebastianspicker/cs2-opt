...
pub(crate) enum Command {
    ...
    /// Back up, deploy, and verify one or more embedded optional CS2 CFGs.
    Cs2Cfg {
        #[arg(long = "asset", value_parser = parse_optional_cfg_asset, action = ArgAction::Append, required = true)]
    ...
    /// Evaluate and optionally persist an explicit raw-latency or VRR cap strategy.
    FpsCap {
        ...
        /// Raw mode: zero means uncapped; a nonzero cap requires measured P1 headroom.
        #[arg(long, default_value_t = 0)]
        measured_cap: u32,
        /// VRR mode: the display refresh rate used to derive the ceiling.
        #[arg(long, default_value_t = 0)]
        refresh_hz: u32,
...
pub(crate) enum DriverCommand {
    ...
    Plan {
        /// JSON file containing a frametime-domain driver DriverPlanInput record.
        #[arg(long)]
        input: PathBuf,
    },
...
fn parse_optional_cfg_asset(value: &str) -> Result<OptionalCfgAsset, String> {
    OptionalCfgAsset::from_cli_token(value).ok_or_else(|| {
        let accepted = OptionalCfgAsset::ALL
            .iter()
            .map(|asset| asset.cli_token())
            .collect::<Vec<_>>()
            .join(", ");
        format!("unknown optional CS2 CFG asset {value:?}; expected one of: {accepted}")
    })
}
...
pub(crate) struct FpsRequest {
    ...
    pub(crate) strategy: FpsStrategyValue,
    pub(crate) measured_cap: u32,
    pub(crate) refresh_hz: u32,
    pub(crate) ceiling_margin_hz: u32,
...
#[derive(Debug, Clone, Copy, ValueEnum)]
pub(crate) enum FpsStrategyValue {
    Raw,
    Vrr,
}
...
mod tests {
    ...
    #[test]
    fn every_closed_cfg_asset_has_one_cli_token() {
        let tokens = OptionalCfgAsset::ALL
            .into_iter()
            .map(OptionalCfgAsset::cli_token)
            .collect::<BTreeSet<_>>();
        assert_eq!(tokens.len(), OptionalCfgAsset::ALL.len());
    }

    #[test]
    fn cs2_cfg_accepts_repeatable_closed_assets() {
        ...
        .expect("closed CFG command");
        let Some(Command::Cs2Cfg { assets, yes }) = cli.command else {
            panic!("expected cs2-cfg command");
        };
        assert_eq!(
            assets,
            vec![
                OptionalCfgAsset::NetStable,
                OptionalCfgAsset::Competitive2026
            ]
        );
        assert!(yes);
    }
