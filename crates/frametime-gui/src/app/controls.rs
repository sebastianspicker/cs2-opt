...
pub(super) fn create_controls(
    ...
    let cs2_cfg = create_cs2_cfg_controls(parent, instance)?;
...
fn create_benchmark_controls(
    ...
    let fps_label = create_static_text(parent, instance, "Evidence", 0)?;
    let fps_input = create_text_control(
        parent,
        instance,
        "EDIT",
        "Use >=5 VProf runs below",
        WS_CHILD
            | WS_BORDER
    ...
    let min_label = create_static_text(parent, instance, "Target raw cap (0 = uncapped)", 0)?;
...
fn create_video_controls(
    ...
    let video_tier_label = create_static_text(parent, instance, "Presentation goal", 0)?;
...
fn create_cs2_cfg_controls(
    parent: HWND,
    instance: HINSTANCE,
) -> windows::core::Result<Cs2CfgControls> {
    let cs2_cfg_asset_label = create_static_text(parent, instance, "Packaged CFG asset", 0)?;
    let cs2_cfg_asset = create_text_control(
        parent,
        instance,
        "COMBOBOX",
        "",
